...
void outputPair(const std::vector<std::pair<TT, TT>> &matrix, const string &fileName) {
...

    {
        for (const auto &pair: matrix) {
            outFile << std::setprecision(8) << pair.first
...
vector<TT> CalcGaussMethod(vector<vector<TT>> matr, vector<TT> vect) {
    ...
    for (size_t k = 0; k < matr.size(); ++k) {
        size_t maxValInd = k;
        for (size_t i = k; i < matr[0].size(); ++i) {
    ...
    for (int i = matr.size() - 1; i >= 0; --i) {
        TT sum = 0.0;
        for (size_t j = i + 1; j < matr[0].size(); ++j) {
